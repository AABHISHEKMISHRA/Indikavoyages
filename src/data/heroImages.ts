export const HERO_IMAGES = [
  {
    src: "/images/hero/01-taj-mahal.webp",
    alt: "Ivory marble architecture and gardens (Agra, India).",
  },
  {
    src: "/images/hero/02-kerala-backwaters.webp",
    alt: "Calm backwaters lined with palm trees (Kerala, India).",
  },
  {
    src: "/images/hero/03-himalaya.webp",
    alt: "Snow-capped peaks above clouds (Himalayas).",
  },
  {
    src: "/images/hero/hawa-mahal-jaipur.webp",
    alt: "Hawa Mahal (Palace of Winds) facade, Jaipur, India.",
  },
  {
    src: "/images/hero/05-varanasi-ganga.webp",
    alt: "River ghats along the Ganges (Varanasi, India).",
  },
  {
    src: "/images/hero/06-amer-fort-distance.webp",
    alt: "Fort above a lake at daylight (Amer, India).",
  },
  {
    src: "/images/hero/07-amer-fort-reflection.webp",
    alt: "Fort reflected in still water (Amer, India).",
  },
  {
    src: "/images/hero/08-spice-jars.webp",
    alt: "Spice jars in a market display (India).",
  },
  {
    src: "/images/hero/09-spice-spoons.webp",
    alt: "Spices on spoons (India).",
  },
] as const;
